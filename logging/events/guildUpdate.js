...
client.on('guildUpdate', async (oldGuild, newGuild) => {
    try {
        const enabled = daalbot.fs.read(path.resolve(`./db/logging/${newGuild.id}/GUILDUPDATE.enabled`), 'utf8');
        if (enabled == 'true') {
            if (!fs.existsSync(`./db/logging/${newGuild.id}/channel.id`)) return;

            const channelID = daalbot.fs.read(path.resolve(`./db/logging/${newGuild.id}/channel.id`), 'utf8');
