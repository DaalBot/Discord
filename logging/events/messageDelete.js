...
client.on('messageDelete', async (message) => {
    try {
        ...
        const enabled = daalbot.fs.read(path.resolve(`./db/logging/${message.guild.id}/MESSAGEDELETE.enabled`), 'utf8');
        if (enabled == 'true') {
            if (fs.existsSync(path.resolve(`./db/logging/${message.channel.guild.id}/MESSAGEDELETE.exclude`))) {
                const excluded = daalbot.fs.read(path.resolve(`./db/logging/${message.channel.guild.id}/MESSAGEDELETE.exclude`), 'utf8').split('\n');
            ...
            const channelID = daalbot.fs.read(path.resolve(`./db/logging/${message.guild.id}/channel.id`), 'utf8');
